...
class FusionAdapter(nn.Module):
    ...
    def forward(self, feat1: torch.Tensor, feat2: torch.Tensor) -> torch.Tensor:
        ...
        out = seq.transpose(1, 2).view(B, C, H, W)
        return out
...
class ResNetSideViTClassifier_MLP_CNNVIT(nn.Module):
    def __init__(
        ...
        super().__init__()
        # 1) Load EfficientNetV2 backbone with intermediate feature maps
        self.backbone = timm.create_model(backbone, pretrained=pretrained, features_only=True)
        feat_dims = self.backbone.feature_info.channels()  # e.g. [24, 48, 64, 128] for v2_s
        c1, c2, c3, c4 = feat_dims

        # 2) Freeze stem and first two block groups for general features
        if hasattr(self.backbone, 'stem'):
            for p in self.backbone.stem.parameters():
                p.requires_grad = False
        if hasattr(self.backbone, 'blocks'):
            for blk in self.backbone.blocks[:2]:  # freeze first half of stages
                for p in blk.parameters():
                    p.requires_grad = False

        # 3) Fusion adapters linking features for Side-ViTs
        self.fuse12 = FusionAdapter(c2, c3, fusion_dim=c3)
        ...
        # 5) Black-box Side-ViT modules
        self.sidevit1    = side_vit1
        self.sidevit2    = side_vit2
        self.sidevit_cnn = side_vit_cnn

        # 6) Final classification head
        total_dim = (side_vit1.out_dim + side_vit2.out_dim + side_vit_cnn.out_dim)
        hidden_dim = getattr(cfg, 'mlp_hidden_dim', 512)
